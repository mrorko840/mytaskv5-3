...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $widget['active_user']             = User::where('plan_id', '>', 0)->count();
        ...
        $today['deposit'] = Deposit::successful()->whereDate('created_at', Carbon::today())->sum('amount');
        $today['withdraw'] = Withdrawal::approved()->whereDate('created_at', Carbon::today())->sum('amount');
        $today['commission'] = CommissionLog::whereDate('created_at', Carbon::today())->sum('amount');
        $total['user_balance'] = User::sum('balance');
        $total['admin_add'] = Transaction::where('remark', 'balance_add')->sum('amount');
        $total['admin_cut'] = Transaction::where('remark', 'balance_subtract')->sum('amount');

        return view('admin.dashboard', compact('pageTitle','today','total', 'widget', 'chart','deposit','withdrawals','report','depositsMonth','withdrawalMonth','months','trxReport','plusTrx','minusTrx','widget'));
    }
    ...
    public function ajaxNotify(){
        $all_admin_notify = AdminNotification::where('read_status',0)->with('user')->orderBy('id','desc')->take(10)->get();
        $all_admin_notify_count = AdminNotification::where('read_status',0)->count();

        return response()->json(['all_admin_notify'=>$all_admin_notify, 'all_admin_notify_count'=>$all_admin_notify_count ]);
    }
