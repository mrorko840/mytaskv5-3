...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        $viewShare['AllUsers'] = User::where('plan_id', '!=', 0)->paginate(getPaginate());
        $viewShare['withdrawMethod'] = WithdrawMethod::where('status',1)->get();
        $viewShare['gatewayCurrency'] = GatewayCurrency::whereHas('method', function ($gate) { $gate->where('status', 1);})->with('method')->orderby('method_code')->get();
